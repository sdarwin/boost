...
if test "x$TOOLSET" = x; then
  guessed_toolset=`CXX= CXXFLAGS= LINKFLAGS= B2_VERBOSE=1 $my_dir/tools/build/src/engine/build.sh --guess-toolset`
...
if test "x$BJAM" = x; then
  ...
  pwd=`pwd`
  (cd "$my_dir/tools/build/src/engine" && CXX= CXXFLAGS= LINKFLAGS= B2_VERBOSE=1 ./build.sh)
  if [ $? -ne 0 ]; then
      echo
