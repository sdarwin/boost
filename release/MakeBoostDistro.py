...
source_file_patterns = [
    '*.jam', 'Jamfile.v2', 'Jamfile', 'Jamfile.jam', 'jamfile.jam', 'jamfile.v2', 'jamfile', 'build.jam', 'Jamroot',
    "*.cpp", "*.hpp", "*.ipp", "*.cxx", "*.hxx", "*.c", "*.h", "*.asm", "*.S", "CMakeLists.txt", "*.cmake", "Makefile",
    "*.py", "*.sh", "*.bash", "*.zsh", "*.ksh", "*.csh", "*.bat", "*.pl",
    "INSTALL", "LICENSE*", "COPYING*", "COPYRIGHT*", "LICENCE*", "UNLICENSE*"]
...
def is_html_content_file(file_path):
    return any(file_path.endswith(ext) for ext in
               ['.html', '.htm', '.css', '.js', '.txt', 'README.md', '.pdf', 'readme', '.md']) and \
        os.path.split(file_path)[1] != 'CMakeLists.txt'


def is_source_file(f):
    return any(fnmatch.fnmatch(os.path.split(f)[1], pattern) for pattern in source_file_patterns)
...
def is_doc_file(f):
    return is_html_content_file(f) or is_image_file(f) or any(
        fnmatch.fnmatch(os.path.split(f)[1], pattern) for pattern in DocFiles)
...
def copy_distro_file(s, d, f, include_source, include_docs):
    if os.path.isfile(os.path.join(s, f)) and not should_ignore_file(s, f) and (
            (include_source and is_source_file(f)) or (include_docs and is_doc_file(f))):
...
def copy_distro_dir(s, d, dd, include_source, include_docs):
    if os.path.isdir(os.path.join(s, dd)) and not should_ignore_file(s, dd):
        if include_source and include_docs:
            if os.path.isdir(os.path.join(s, dd)) and not should_ignore_file(s, dd):
                shutil.copytree(os.path.join(s, dd), os.path.join(d, dd), symlinks=False, ignore=ignored_files_pattern)
...
def copy_distro_subproject(src, dst, headers, p, include_source, include_docs):
    ...
    for item in os.listdir(source_project_dir):
        ...
        if os.path.isfile(os.path.join(source_project_dir, item)):
            copy_distro_file(source_project_dir, dest_project_dir, item, include_source, include_docs)
        elif item != "include":
            copy_distro_dir(source_project_dir, dest_project_dir, item, include_source, include_docs)

    # Now the includes
    source_project_dir = os.path.join(src, "%s/include/boost" % p)
    if os.path.exists(source_project_dir):
        copy_distro_include_dir(source_project_dir, headers, include_source, include_docs)
...
def copy_distro_nested_project(src, dst, headers, p, include_source, include_docs):
    ...
    for item in os.listdir(source_nested_dir):
        if os.path.isfile(os.path.join(source_nested_dir, item)):
            copy_distro_file(source_nested_dir, dest_nested_dir, item, include_source, include_docs)
        elif item != "include":
            copy_distro_dir(source_nested_dir, dest_nested_dir, item, include_source, include_docs)
...
def main(src_root, dest_root, include_source=True, include_docs=True, cmake_distro=False):
    ...
    for f in os.listdir(source_libs_dir):
        copy_distro_file(source_libs_dir, dest_libs_dir, f, include_source, include_docs)
    ...
    for f in os.listdir(source_libs_dir):
        if os.path.isdir(os.path.join(source_libs_dir, f)):
            if os.path.isfile(os.path.join(source_libs_dir, f, "meta", "libraries.json")):
                ...
                for s in os.listdir(os.path.join(source_libs_dir, f)):
                    if os.path.isdir(os.path.join(source_libs_dir, f, s)):
                        if os.path.isfile(os.path.join(source_libs_dir, f, s, "meta", "libraries.json")):
                            boost_subprojects.add((f, s))
                        elif os.path.isdir(os.path.join(source_libs_dir, f, s, "include")):
                            boost_subprojects.add((f, s))
    ...
    if not cmake_distro:
        ...
        for p in boost_subprojects:
            if isinstance(p, six.string_types):
                copy_distro_subproject(source_libs_dir, dest_libs_dir, dest_headers, p, include_source, include_docs)
            else:
                ...
                for f in os.listdir(nested_source_dir):
                    copy_distro_file(nested_source_dir, nested_dest_dir, f, include_source, include_docs)
                copy_distro_nested_project(nested_source_dir, nested_dest_dir, nested_dest_headers_dir, p,
                                           include_source,
                                           include_docs)
    else:
        # Step 4b) Clean tests dir
                        ...
                        try:
                            ...
                            for href in hrefs:
                                ...
                                if os.path.exists(dest):
                                    continue
                                src = os.path.join(src_root, os.path.relpath(dest, dest_root))
...
if __name__ == "__main__":
    main(sys.argv[1], sys.argv[2], sys.argv[3] if len(sys.argv) > 3 else True,
         sys.argv[4] if len(sys.argv) > 4 else True, sys.argv[5] if len(sys.argv) > 5 else False)
