...
class parallel_call(threading.Thread):
    ...
    def join(self, **kwargs):
        super(parallel_call, self).join(**kwargs)
        if self.result != 0:
    ...
    def main(self):
        for action in self.actions:
            ...
            if hasattr(self, action_m):
                ...
                utils.log('### %s not available in %s' % (action, self.__class__.__name__))
...
class ci_cli():
    ...
    def command_clone(self):
        ...
        if not os.path.exists(os.path.join(self.script.root_dir, '.git')):
            utils.check_call("git", "clone", "--depth=50", "--branch=%s" % self.script.branch,
                             "https://github.com/boostorg/boost.git", "boostorg/boost")
