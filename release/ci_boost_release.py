...
def generatehtmlpages(source_dir, build_dir):
    ...
    boostlibrariestoadd = ["libs/numeric/conversion", "libs/numeric/interval", "libs/numeric/odeint",
                           "libs/numeric/ublas", "libs/spirit/classic", "libs/spirit/repository"]
    ...
    for directoryname in glob.iglob('libs/*', recursive=False):
        if os.path.isdir(directoryname) and (directoryname not in boostlibrariestoskip) and os.path.isfile(
                directoryname + "/meta/libraries.json"):  # filter dirs
...
"""
        ...
        data = rtemplate.render(release_notes_url=release_notes_url, version=source_version, allmetadata=allmetadata,
                                boostlibrarycategories=boostlibrarycategories)

        # Post processing
        data = data.replace('Determines the type of a function call expression, from ',
                            'Determines the type of a function call expression')
        data = data.replace('idiosyncrasies; not intended for library users, from ',
...
class script(script_common):
    ...
    def __init__(self, ci_klass, **kargs):
        self.mode = None
        self.eol = None
        self.host_dependencies = None
        self.python_version = None
        self.pip_version = None
        self.gem_version = None
        self.docutils_version = None
        self.sphinx_version = None
        self.asciidoctor_version = None
        self.latex_version = None
    ...
    def init(self, opt, kargs):
        ...
        opt.add_option('--host-dependencies',
                       help='skip system-wide dependency installation and use dependencies from the host')
        self.host_dependencies = os.getenv('HOST_DEPENDENCIES', False)

        opt.add_option('--build-dir',
                       help='override the build dir (default ../build)',
                       default=None)
        self.build_dir = None

        opt.add_option('--releases-dir',
                       help='override the release dir (default ..)',
                       default=None)
    ...
    def command_install(self):
        ...
        if not self.host_dependencies:
            self.command_install_docutils()
            self.command_install_sphinx()
            self.command_install_asciidoctor()
        else:
            self.python_version = self.check_python()
            self.pip_version = self.check_pip()
            self.gem_version = self.check_gem()
            self.latex_version = self.check_latex()
            self.docutils_version = self.check_docutils()
            self.sphinx_version = self.check_sphinx()
            self.asciidoctor_version = self.check_asciidoctor()
    ...
    def command_install_rapidxml(self):
        ...
        if not os.path.exists(os.path.join(self.build_dir, 'rapidxml.zip')):
            utils.check_call("wget", "-O", "rapidxml.zip",
    ...
    def check_docutils(self):
        # build scripts expect the specified location
        if self.python_version is None:
            return None
        if not os.path.exists('/usr/share/docutils/tools/rst2html.py'):
            utils.log('docutils 0.12 script not found at /usr/share/docutils/tools/rst2html.py')
            utils.log('please install it with: pip install --user docutils==0.12')
            return False
        try:
            return self.check_pip_version('docutils', (0, 12, 0))
        except ImportError:
            return None
    ...
    def command_install_sphinx(self):
        ...
        utils.make_file(os.path.join(self.build_dir, 'constraints.txt'), 'Sphinx==1.6.1')
        utils.check_call("pip", "install", "--user", "Sphinx==1.6.1")
        utils.check_call("pip", "install", "--user", "sphinx-boost==0.0.3")
        utils.check_call("pip", "install", "--user", "-c", os.path.join(self.build_dir, 'constraints.txt'),
    ...
    def check_sphinx(self):
        try:
            if self.python_version is None:
                return None
            if self.python_version[0] == 3 and self.python_version[1] == 10:
                utils.log('Sphix 1.6.1 is not compatible with python 3.10')
                return None
            v = self.check_pip_version('Sphinx', (1, 6, 1))
            if self.check_pip_version('sphinx-boost', (0, 0, 3)) is None:
                return None
            return v
        except ImportError:
            return None
    ...
    def command_install_docbook(self):
        ...
        if not os.path.exists(os.path.join(self.build_dir, 'docbook-xsl.zip')):
            utils.check_call("wget", "-O", "docbook-xsl.zip",
    ...
    def command_install_asciidoctor(self):
        ...
        if os.geteuid() != 0:
            ...
            os.environ["PATH"] += os.pathsep + os.path.join(self.build_dir, '.gems', 'bin')
    ...
    def check_asciidoctor(self):
        try:
            if self.gem_version is None:
                return None
            if self.python_version is None:
                return None
            v = self.check_gem_version("asciidoctor", (1, 5, 8))
            if v is None:
                return None
            if self.check_gem_version("rouge", (4, 1, 0)) is None:
                return None
            if pythonversion == "2":
                if self.check_gem_version("pygments.rb", (1, 2, 1)) is None:
                    return None
            else:
                if self.check_gem_version("pygments.rb", (2, 1, 0)) is None:
                    return None
            if self.check_pip_version('Pygments', (2, 1, 0)) is None:
                return None
            if self.check_pip_version('jam_pygments', (0, 2, 0), message=False) is None:
                utils.log('jam-pygments not found')
                utils.log(
                    'please install it with: pip install --user https://github.com/bfgroup/jam_pygments/archive/master.zip')
                return None
            return v
        except ImportError:
            return None

    def check_gem_version(self, name, version):
        output = subprocess.check_output(["gem", "list", name]).decode("utf-8")
        version_line = None
        name_v_prefix = name + ' ('
        for line in output.splitlines():
            if line.startswith(name_v_prefix) and line.endswith(')'):
                version_line = line
                break
        if version_line is not None:
            gem_versions = version_line[len(name_v_prefix): -1].split(',')
            compatible_gem_version = None
            for gem_version in gem_versions:
                if gem_version.startswith('default: '):
                    gem_version = gem_version[9:]
                gem_versions = self.parse_semver(gem_version.strip())
                if self.is_semver_compatible(gem_versions, version):
                    return gem_version
        utils.log('%s %s.%s.%s not found' % (name, version[0], version[1], version[2]))
        utils.log('Installing locally in %s' % self.build_dir)
        if "GEM_HOME" not in os.environ or not os.environ["GEM_HOME"]:
            os.environ["GEM_HOME"] = os.path.join(self.build_dir, '.gems')
            os.environ["PATH"] += os.pathsep + os.path.join(self.build_dir, '.gems', 'bin')
        utils.check_call("gem", "install", name, "--version", "%s.%s.%s" % (version[0], version[1], version[2]))
        return version

    def check_pip_version(self, name, version, message=True):
        try:
            output = subprocess.check_output(["pip", "show", name]).decode("utf-8")
            version_str = None
            install_name = name
            for line in output.splitlines():
                if line.startswith("Name:"):
                    install_name = line.split(":")[1].strip()
                if line.startswith("Version:"):
                    version_str = line.split(":")[1].strip()
                    break
            if version_str is None:
                if message:
                    utils.log('%s %s.%s.%s not found' % (install_name, version[0], version[1], version[2]))
                    utils.log('please install it with: pip install --user %s=%s.%s.%s' % (
                        install_name, version[0], version[1], version[2]))
                return None
            pip_version = self.parse_semver(version_str)
            r = self.is_semver_compatible(pip_version, version)
            if not r:
                if message:
                    utils.log('%s %s.%s.%s not found' % (install_name, version[0], version[1], version[2]))
                    utils.log('please install it with: pip install --user %s=%s.%s.%s' % (
    ...
    def check_python(self):
        output = subprocess.check_output([pythonbinary, "--version"]).decode("utf-8")
        for line in output.splitlines():
            if line.startswith('Python '):
                return self.parse_semver(line.split()[1].strip())
        utils.log('python unavailable')
        utils.log('Please install it with: apt-get install python3 python-is-python3')
        return None
    ...
    def check_pip(self):
        ...
        for line in output.splitlines():
            if line.startswith('pip'):
                return self.parse_semver(line.split()[1].strip())
        utils.log('pip unavailable')
        utils.log('Please install it with: apt-get install python3-pip')
        return None

    def check_gem(self):
        output = subprocess.check_output(["gem", "--version"]).decode("utf-8")
        for line in output.splitlines():
            if line != '':
                return self.parse_semver(line.strip())
        if os.geteuid() != 0 and ("GEM_HOME" not in os.environ or not os.environ["GEM_HOME"]):
            os.environ["GEM_HOME"] = os.path.join(self.build_dir, '.gems')
            os.environ["PATH"] += os.pathsep + os.path.join(self.build_dir, '.gems', 'bin')
        utils.log('gem unavailable')
        utils.log('Please install it with: apt-get install rubygems')
        return None

    def check_latex(self):
        output = subprocess.check_output(["latex", "--version"]).decode("utf-8")
        for line in output.splitlines():
            if line.startswith('pdfTeX'):
                return self.parse_semver(line.split()[1].strip().split('-')[0].strip())
        utils.log('latex unavailable')
        utils.log('Please install it with: apt-get install texlive-latex-base')
        return None
    ...
    def command_before_build(self):
        ...
        # EOL style.
        utils.check_call("git", "config", "--global", "core.eol", "lf" if self.eol == 'LF' else 'crlf')
        utils.check_call("git", "config", "--global", "core.autocrlf", "input" if self.eol == 'LF' else 'true')
        utils.check_call("git", "rm", "--quiet", "--cache", "-r", ".")
        utils.check_call("git", "reset", "--quiet", "--hard", "HEAD")
        utils.check_call("git", "submodule", "--quiet", "foreach", "--recursive", "git", "rm", "--quiet", "--cache",
                         "-r", ".")
        utils.check_call("git", "submodule", "--quiet", "foreach", "--recursive", "git", "reset", "--quiet", "--hard",
                         "HEAD")
    ...
    def command_build(self):
        ...
        self.b2('-q', '-d0', '--build-dir=%s' % (self.build_dir),
                ...
                'tools/quickbook',
                'tools/boostdep/build',
                'tools/auto_index/build' if enable_auto_index else '')
        shutil.copy2(os.path.join(self.root_dir, 'dist', 'bin', 'boostdep'),
                     os.path.join(self.build_dir, 'dist', 'bin', 'boostdep'))
        ...
        if not self.host_dependencies:
            ...
            utils.check_call("git", "clean", "-dfqx")
            os.chdir(os.path.join(self.root_dir, "tools", "boostdep"))
            utils.check_call("git", "clean", "-dfqx")

        # Set up build config.
        docutils_path = '/usr/share/docutils'
        if self.host_dependencies and self.docutils_version == None:
            docutils_path = ''
        utils.make_file(os.path.join(self.build_dir, 'site-config.jam'),
                        ...
                        'using auto-index : "%s" ;' % (
                            os.path.join(self.build_dir, 'dist', 'bin', 'auto_index')) if enable_auto_index else '',
        ...
        if self.host_dependencies:
            if self.asciidoctor_version is None:
                exclude_libraries.update(
                    ['qvm', 'assert', 'container_hash', 'system', 'describe', 'endian', 'unordered',
                     'io', 'lambda2', 'leaf', 'mp11', 'smart_ptr', 'throw_exception', 'variant2'])
            if self.docutils_version is None:
                exclude_libraries.update(['parameter', 'geometry', 'parameter_python', 'python'])
            if self.sphinx_version is None:
                exclude_libraries.update(['hof', 'gil', 'python'])
            if self.latex_version is None:
                exclude_libraries.update(['accumulators'])
        ...
        if self.branch == 'master':
            # Checkout develop while script ecosystem in development
            # Scripts still missing in master
        ...
        if not os.path.exists(antora_dir):
            utils.check_call("git", "clone", "--depth=1", "--branch=%s" % checkout_branch,
                             "https://github.com/CPPAlliance/site-docs.git", "antora")
        os.chdir(antora_dir)
        ...
        MakeBoostDistro.main(self.root_dir, "%s-source" % self.boost_release_name, True, False)
        MakeBoostDistro.main(self.root_dir, "%s-docs" % self.boost_release_name, False, True)
        MakeBoostDistro.main(self.root_dir, "%s-cmake" % self.boost_release_name, True, False, True)

        # Patch release with generated html
        for release_name in [self.boost_release_name, "%s-docs" % self.boost_release_name]:
            for sourcefilename in ["index.html", "libs/libraries.htm"]:
                shutil.copyfile(os.path.join(self.build_dir, sourcefilename),
                                os.path.join(self.releases_dir, release_name, sourcefilename))

        # Patch with antora docs
        for release_name in [self.boost_release_name, "%s-docs" % self.boost_release_name]:
            ...
            shutil.copytree(antora_lib_docs, release_lib_docs, symlinks=False, ignore=MakeBoostDistro.ignored_files_pattern,
                            dirs_exist_ok=True)


        packages = []
        archive_files = []
        for release_name in [self.boost_release_name, "%s-source" % self.boost_release_name,
                             "%s-docs" % self.boost_release_name, "%s-cmake" % self.boost_release_name]:
            ...
            if self.eol == 'LF':
                ...
                packages.append(parallel_call(
                    ...
                    release_name))
                archive_files.append(
                    '%s%s.tar.xz' % (release_name, self.archive_tag))
                packages.append(parallel_call(
                    'tar', '--exclude=ci_boost_common.py', '--exclude=ci_boost_release.py', '-jcf',
                    '%s%s.tar.xz' % (release_name, self.archive_tag),
                    release_name))
            ...
            if self.eol == 'CRLF':
                ...
                packages.append(parallel_call(
                    'zip', '-qr', '-9',
                    '%s%s.zip' % (release_name, self.archive_tag),
                    release_name, '-x', release_name + '/ci_boost_common.py',
                    release_name + '/ci_boost_release.py'))
                archive_files.append(
                    '%s%s.7z' % (release_name, self.archive_tag))
                with open('/dev/null') as dev_null:
                    utils.check_call(
                        '7z', 'a', '-bd', '-mx=7', '-ms=on', '-x!' + release_name + '/ci_boost_common.py',
                                                             '-x!' + release_name + '/ci_boost_release.py',
                                                             '%s%s.7z' % (release_name, self.archive_tag),
                        release_name, stdout=dev_null)
        ...
        for archive_file in archive_files:
            ...
            try:
                created_date = subprocess.check_output(
                    "set -e && set -o pipefail; TZ=UTC git -C " + self.root_dir + " show -s --date='format-local:%Y-%m-%dT%H:%M:%SZ' " + self.commit + " | grep Date: | tr -s ' ' | cut -d' ' -f 2 ",
    ...
    def command_after_success(self):
        ...
        # Skip this step until we decide where files should be uploaded for the
        # new website
        return
